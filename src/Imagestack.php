...
class Imagestack
{
  ...
  const
    ...
    ERROR_GD_NOT_ENABLED = 2,
    ERROR_INVALID_DATA_URI = 3,
    ERROR_INVALID_IMAGE = 4,
    ERROR_UNSUPPORTED_FORMAT = 5,
    ERROR_WEBP_NOT_ENABLED = 6,
    ERROR_WRITE = 7,
    ERROR_CROP_VALUES = 8;
  ...
  public function __construct($image = null)
  {
    ...
    if (extension_loaded('gd')) { // Check for the required GD extension
      ...
      throw new \Exception('Required extension GD is not loaded.', self::ERROR_GD_NOT_ENABLED);
    }
  ...
  public function fromHttpFile($uri)
  {
    ...
    if (!$this->image) {
      throw new \Exception("Invalid image data.", self::ERROR_INVALID_IMAGE);
    }
  ...
  public function fromLocalFile($file)
  {
    ...
    $handle = @fopen($file, 'r');
    if ($handle === false) {
      throw new \Exception("File not found: ${file}", self::ERROR_FILE_NOT_FOUND);
    }
    fclose($handle);
    ...
    if ($info === false) {
      throw new \Exception("Invalid image file: ${file}", self::ERROR_INVALID_IMAGE);
    }
    ...
    if (!$this->image) {
      throw new \Exception("Unsupported format: " . $this->mimeType, self::ERROR_UNSUPPORTED_FORMAT);
    }
  ...
  protected function generate($mimeType = null, $quality = 100)
  {
    ...
    switch ($mimeType) {
      ...
      case 'image/x-windows-bmp':
        ...
        break;
      default:
        throw new \Exception('Unsupported format: ' . $mimeType, self::ERROR_UNSUPPORTED_FORMAT);
    }
  ...
  public function saveFile($file, $mimeType = null, $quality = 100)
  {
    ...
    if (!file_put_contents($file, $image['data'])) {
      throw new \Exception("Failed to write image to file: ${file}", self::ERROR_WRITE);
    }
  ...
  {
    if ($y1 >= $y2 || $x1 >= $x2)
      throw new \Exception('Invalid crop values $x2 must be bigger tha $x1 and $y2 must be bigger than $y1 . ', self::ERROR_CROP_VALUES);
